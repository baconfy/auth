...
@section('welcome', __('auth::confirm.welcome'))
...
@section('content')
    <section class="mt-1">
        <form class="form" method="POST" action="{{ route('password.confirm') }}">
            ...
            <button class="block w-full rounded bg-primary text-white p-3 mt-4 mb-8" type="submit">{{ __('auth::confirm.action') }}</button>
        </form>
