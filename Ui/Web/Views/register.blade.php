    ...
    <section class="mt-1">
        <form class="form" method="POST" action="{{ route('register') }}">
            ...
            <div class="input @error('name') border-red-500 @enderror">
                <label class="@error('name') text-red-500 @enderror" for="name">{{ __('auth::register.name') }}</label>
            ...
            <button class="block w-full rounded bg-primary text-white p-3 mt-4 mb-8" type="submit">{{ __('auth::register.action') }}</button>
        </form>

        <div class="divider"></div>

        <p class="w-full text-xs text-center text-gray-700 mt-8 -mb-4">
            {{ __('auth::register.already-account') }} <a class="text-primary hover:text-blue-700 no-underline" href="{{ route('login') }}">{{ __('auth::register.back-action') }}</a>
        </p>
    </section>
@endsection
